...
func (p *Parser) VarsDecl(typ constants.ItemType) []nd.IdentNode {
	...
	for p.currentToken.TokType != constants.SEMI {
		...
		if p.Exists(var_name) { // Это не совсем верно т.к. если я захочу в FOR или IF создать внутри переменную с тем же именем, то я пойду нахер
...
func (p *Parser) Operators() nd.Node {
	...
	if p.currentFunction == "global" {
		...
		case constants.IFT:
			...
			if condition_llirVal.Type().Equal(types.I32) || condition_llirVal.Type().Equal(types.I32Ptr) {
				condition_llirVal = p.gen.B.NewICmp(enum.IPredSGT, condition_llirVal, constant.NewInt(types.I32, 0))
			}
			...
			if len(if_operators) > 0 {
				if _, ok := if_operators[len(if_operators)-1].(nd.ReturnNode); !ok {
			...
			if len(else_operators) > 0 {
				if _, ok := else_operators[len(else_operators)-1].(nd.ReturnNode); !ok {
					p.gen.B.NewBr(end_block)
				}
			} else {
				p.gen.B.NewBr(end_block)
		...
		case constants.IDENT:
			if _, ok := p.sts[p.currentToken.Text]; ok {
				...
				case nd.BooleanOpNode:
					Id_n.LLIRVal = p.gen.B.NewBitCast(Id_n.LLIRVal, types.I32Ptr)
					p.gen.B.NewStore(right.LLIRVal, Id_n.LLIRVal)
		...
		case constants.RETURNT:
			...
			case nd.IdentNode:
				...
				p.gen.B.NewRet(llirVal)

				p.Eat(constants.SEMI)
				new_ret_node := nd.NewReturnNode(node)
				return new_ret_node
...
func (p *Parser) LogicAND() nd.Node {
	node := p.Comparison()
...
func (p *Parser) Comparison() nd.Node {
	node := p.Expression()
	...
	for p.currentToken.TokType == constants.COMPARATIVE {
		...
		var comp_inst value.Value
		comp_inst = p.gen.B.NewICmp(predicate, left_llirVal, right_llirVal)
		if comp_inst.Type() == types.I1 || comp_inst.Type() == types.I1Ptr {
			comp_inst = p.gen.B.NewZExt(comp_inst, types.I32)
		}
